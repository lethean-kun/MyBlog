                ...
                <div>
					...
					<c:if test="${allTopic!=null}">
						<c:forEach items="${allTopic }" var="topic" varStatus="status" >
			                <div class="bw_text">
			                    <span class="title_span"><a class="bw_text_title" href="${pageContext.request.contextPath }/topicInfo/${topic.topicid}">${topic.title}</a><br>(${topic.addtime })<a style="float: right;" class="bw_text_title_bt" href="###" target="_self">作者：${topic.user.username}</a></span>
			                    <br />
			                    ...
			                    <div class="text_text"><p id="su${status.index }">&nbsp;&nbsp;${topic.contents}</p></div>
						...
						<script>
							...
							var oP = document.getElementById('su${status.index}');
							var allContent = oP.innerHTML; //存放所有内容
							oP.innerHTML = oP.innerHTML.substr(0, 150); //截取前100个字符
						</script>		
		                </c:forEach>
